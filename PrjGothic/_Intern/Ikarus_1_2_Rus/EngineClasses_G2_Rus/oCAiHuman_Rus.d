//######################################################
//
//  Ќет никаких гарантий относительно этих двух классов.
//  oCAniCtrl_Human показалась мне совершенно бесполезной 
//  так что € не стал тратить на нее много сил.
//  zCAIPlayer более значим и, как минимум, 
//  свойства из "Species.d" будут интересны
//  дл€ модостроителей. Ќапример, можно сделать заклинание
//  "хождени€ по воде" или заставить игрока подниматьс€
//  по отвесным стенам.
//  ѕользуйтесь на свой страх и риск.
//
//######################################################
...
class zCAIPlayer {
    ...
    // Config
    //struct zTConfig {
        // асаетс€ величин, заданных в Species.d.
    ...
    //
	≈сли кто-то взбираетс€ в гору, это еще не значит что такой NPC будет
	наклонен под тем же углом, что и поверхность под ним. ѕо крайней мере, не все NPC.
	«десь приведены константы, которые позвол€ют управл€ть этим.
    
    enum zTSurfaceAlignMode {                       
        zMV_SURFACE_ALIGN_NONE  = 0,                // нет выравнивани€ (напр., человек)
        zMV_SURFACE_ALIGN_NORMAL= 1,                // напр., мракорис
        zMV_SURFACE_ALIGN_HIGH  = 2,                // напр., м€сные жуки, которые всеми ногами сто€т на земле.
    }; */                                               
    
    /*
    //—труктура с информацией о споcобност€х NPC взбиратьс€
   struct zTLedgeInfo{
        zVEC3 point;            // ѕозици€ дл€ хватани€
    ...
    */
    ...
    // Ёффекты шлейфа от оружи€
    var int weaponTrailVob;     //zCVob*
    
    // Ёффект кругов на воде
...
class oCAniCtrl_Human {
    ...
    var int bitfield; //см. структуру в комментари€х выше   
    ...
    //ѕоскольку oCAIHuman (подкласс) добавл€ет не так уж много, здесь € бросил даже говорить об этом:
    //¬о вс€ком случае, у мен€ нет причин думать, что он будет хоть чем-то полезен.
