...
instance DIA_VLK_6_News(C_Info)
{
	nr = 101;
	condition = DIA_VLK_6_News_Condition;
	information = DIA_VLK_6_News_Info;
	permanent = TRUE;
	description = "Есть какие-нибудь новости?";
};

func int DIA_VLK_6_News_Condition()
{
	return TRUE;
};

func void DIA_VLK_6_News_Info()
{
	if (hero.voice == 3)	{
	...
	if (rnd == 0)	{
		AI_Output(self,other,"DIA_VLK_News_06_01");	//Говорят, на материке дела все хуже. Хорошо, что нас это не касается.
	} else if ((rnd == 1) && (Wld_GetDay() <= Competition_LastDay))	{
		AI_Output(self,other,"DIA_VLK_News_06_02");	//Константино объявил конкурс по сбору грибов. Интересно, много ли найдется желающих...
	} else if (rnd == 2)	{
		AI_Output(self,other,"DIA_VLK_News_06_03");	//Зиму обещают холодную.
	} else	{
		AI_Output(self,other,"DIA_VLK_News_06_04");	//Губернатор недавно отбыл на материк. Корнелиус опять будет изображать из себя главного.
	};
};

instance DIA_VLK_6_Konkurs(C_Info)
{
	nr = 102;
	condition = DIA_VLK_6_Konkurs_Condition;
	information = DIA_VLK_6_Konkurs_Info;
	permanent = TRUE;
	description = "Ты участвуешь в конкурсе Константино?";
};

func int DIA_VLK_6_Konkurs_Condition()
{
...
func void DIA_VLK_6_Konkurs_Info()
{
	if (hero.voice == 3)	{
	...
	var int rnd;	rnd = Hlp_Random(4);
	if (rnd == 0)	{
		AI_Output(self,other,"DIA_VLK_Konkurs_06_01");	//Нет. Зачем мне это?
	} else if (rnd == 1)	{
		AI_Output(self,other,"DIA_VLK_Konkurs_06_02");	//Нет. У меня и других дел хватает.
	} else	{
		AI_Output(self,other,"DIA_VLK_Konkurs_06_03");	//А что, похоже, что я тут грибы собираю?
	};
};
...
func void B_AssignAmbientInfos_VLK_6(var C_Npc slf)
{
	...
	dia_vlk_6_news.npc = Hlp_GetInstanceID(slf);
	dia_vlk_6_konkurs.npc = Hlp_GetInstanceID(slf);
};
