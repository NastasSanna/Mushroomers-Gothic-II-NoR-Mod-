...
func void DIA_VLK_17_News_Info()
{
	...
	if (rnd == 0)	{
		...
		AI_Output(self,other,"DIA_VLK_News_17_02");	//Константино объявил конкурс по сбору грибов. Интересно, много ли найдется желающих...
	} else if (rnd == 2)	{
		AI_Output(self,other,"DIA_VLK_News_17_03");	//Зиму обещают холодную.
	} else	{
		AI_Output(self,other,"DIA_VLK_News_17_04");	//Губернатор недавно отбыл на материк. В его отсутствие в ратуше распоряжается Корнелиус.
	};
};
...
func void DIA_VLK_17_Konkurs_Info()
{
	...
	var int rnd;	rnd = Hlp_Random(4);
	if (rnd == 0)	{
		AI_Output(self,other,"DIA_VLK_Konkurs_17_01");	//Нет. Зачем мне это?
	} else if (rnd == 1)	{
		AI_Output(self,other,"DIA_VLK_Konkurs_17_02");	//Нет. У меня и других дел хватает.
	} else	{
		AI_Output(self,other,"DIA_VLK_Konkurs_17_03");	//А что, похоже, что я тут грибы собираю?
	};
};
