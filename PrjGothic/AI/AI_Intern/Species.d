/**************************************************************************
  ...
  ›ÍÁÂÏÔÎˇ ÍÎ‡ÒÒ‡ C_GILVALUES. ‚ ÍÓÚÓÓÏ ÔÓÔËÒ‡Ì˚ Ô‡‡ÏÂÚ˚ ‚ÒÂı „ËÎ¸‰ËÈ.
ŒÔËÒ‡ÌËÂ ÔÓÎÂÈ ÒÏ. ‚ Ó·˙ˇ‚ÎÂÌËË ÍÎ‡ÒÒ‡.
  ƒÎˇ ‚ÒÂı Î˛‰ÂÈ ‚ ÌÂ Á‡‚ËÒËÏÓÒÚË ÓÚ „ËÎ¸‰ËË Ô‡‡ÏÂÚ˚ Á‡‰‡Ì˚ ÔÓ GIL_HUMAN,
‰Îˇ ‚ÒÂı „ËÎ¸‰ËÈ ÏÓÌÒÚÓ‚ ÔÓÔËÒ‡Ì˚ ˇ‚ÌÓ, ÌÓ ÏÓ„ÛÚ ·˚Ú¸ ÒÒÎ˚ÍË Ì‡ ÁÌ‡˜ÂÌËˇ
ÔÓÎÂÈ ÔÂ‰˚‰Û˘ËÈ „ËÎ¸‰ËÈ.
  TIME_INFINITE - ·ÂÒÍÓÌÂ˜ÌÓ ‰ÓÎ„Ó

  —‚ˇÁ‡ÌÌ˚Â Ù‡ÈÎ˚:
 ËÌ‰ÂÍÒ˚ „ËÎ¸‰ËÈ (GIL_x) Ë ÍÎ‡ÒÒ C_GILVALUES  		_Intern\Constants.d
...
instance Gil_Values(C_GILVALUES)
{
	...
	// œ–»«¬¿ÕÕ€… — ≈À≈“ √Œ¡À»Õ¿ (Í‡Í „Ó·ÎËÌ) -------------------------------
	swim_time[GIL_SUMMONED_GOBBO_SKELETON] = swim_time[GIL_GOBBO_SKELETON];
	dive_time[GIL_SUMMONED_GOBBO_SKELETON] = dive_time[GIL_GOBBO_SKELETON];
	water_depth_knee[GIL_SUMMONED_GOBBO_SKELETON] = water_depth_knee[GIL_GOBBO_SKELETON];
	water_depth_chest[GIL_SUMMONED_GOBBO_SKELETON] = water_depth_chest[GIL_GOBBO_SKELETON];
	step_height[GIL_SUMMONED_GOBBO_SKELETON] = step_height[GIL_GOBBO_SKELETON];
	jumplow_height[GIL_SUMMONED_GOBBO_SKELETON] = jumplow_height[GIL_GOBBO_SKELETON];
	jumpmid_height[GIL_SUMMONED_GOBBO_SKELETON] = jumpmid_height[GIL_GOBBO_SKELETON];
	jumpup_height[GIL_SUMMONED_GOBBO_SKELETON] = jumpup_height[GIL_GOBBO_SKELETON];
	slide_angle[GIL_SUMMONED_GOBBO_SKELETON] = slide_angle[GIL_GOBBO_SKELETON];
	turn_speed[GIL_SUMMONED_GOBBO_SKELETON] = turn_speed[GIL_GOBBO_SKELETON];
	climb_heading_angle[GIL_SUMMONED_GOBBO_SKELETON] = climb_heading_angle[GIL_GOBBO_SKELETON];
	climb_horiz_angle[GIL_SUMMONED_GOBBO_SKELETON] = climb_horiz_angle[GIL_GOBBO_SKELETON];
	climb_ground_angle[GIL_SUMMONED_GOBBO_SKELETON] = climb_ground_angle[GIL_GOBBO_SKELETON];
	falldown_height[GIL_SUMMONED_GOBBO_SKELETON] = falldown_height[GIL_GOBBO_SKELETON];
	falldown_damage[GIL_SUMMONED_GOBBO_SKELETON] = falldown_damage[GIL_GOBBO_SKELETON];
	fight_range_fist[GIL_SUMMONED_GOBBO_SKELETON] = fight_range_fist[GIL_GOBBO_SKELETON];
	fight_range_base[GIL_SUMMONED_GOBBO_SKELETON] = fight_range_base[GIL_GOBBO_SKELETON];
	fight_range_g[GIL_SUMMONED_GOBBO_SKELETON] = fight_range_g[GIL_GOBBO_SKELETON];
	...
	// œŒÀ≈¬Œ… ’»ŸÕ»  (√»√¿Õ“— »… ∆” ) -------------------------------------	
	swim_time[GIL_GIANT_BUG] = 40;//TIME_INFINITE;
	...
	// ¬¿–¿Õ (ﬂŸ≈–) --------------------------------------------------------	
	swim_time[GIL_WARAN] = 45;//TIME_INFINITE;
	...
	blood_texture[GIL_SUMMONED_WOLF] = "BQM_Scaveng2.tga";			//NS: ÌÂÚ Ú‡ÍÓÈ ÚÂÍÒÚÛ˚!!!
	...
	//  ¿¡¿Õ ---------------------------------------------------------------
	swim_time[GIL_Keiler] = 110;//TIME_INFINITE;
	dive_time[GIL_Keiler] = 0;
	water_depth_knee[GIL_Keiler] = 30;
	water_depth_chest[GIL_Keiler] = 60;
	step_height[GIL_Keiler] = 50;
	jumplow_height[GIL_Keiler] = 70;
	jumpmid_height[GIL_Keiler] = 150;
	slide_angle[GIL_Keiler] = 50;
	disable_autoroll[GIL_Keiler] = 1;
	surface_align[GIL_Keiler] = 1;
	turn_speed[GIL_Keiler] = 240;
	falldown_height[GIL_Keiler] = 500;
	falldown_damage[GIL_Keiler] = 10;
	fight_range_base[GIL_Keiler] = 170;
	fight_range_fist[GIL_Keiler] = 30;
	fight_range_g[GIL_Keiler] = 100;
	blood_emitter[GIL_Keiler] = "BFX_Preset1";
	blood_texture[GIL_Keiler] = "BQM_Scavenger2.tga";
	blood_flow[GIL_Keiler] = 1;
	blood_amount[GIL_Keiler] = 5;
	blood_max_distance[GIL_Keiler] = 80;
	// œŒÀ«”Õ ( –¿”À≈–) ----------------------------------------------------
	...
	// ÿÕ€√ (À” ≈–) --------------------------------------------------------
	swim_time[GIL_LURKER] = 70;//TIME_INFINITE;
	dive_time[GIL_LURKER] = 70;//TIME_INFINITE;
	...
	// —Õ≈œœ≈– -------------------------------------------------------------
	swim_time[GIL_SNAPPER] = 45;//TIME_INFINITE;
	dive_time[GIL_SNAPPER] = 45;//TIME_INFINITE;
	...
	// Œ√Õ≈ÕÕ€… √ŒÀ≈Ã (Í‡Í Í‡ÏÂÌÌ˚È „ÓÎÂÏ) ---------------------------------
	swim_time[GIL_FIREGOLEM] = 0;
	...
	// Œ–  -----------------------------------------------------------------
	swim_time[GIL_ORC] = 120;//TIME_INFINITE;
	...
	// œ–»«¬¿ÕÕ€…  ¿Ã≈ÕÕ€… —“–¿∆ (ÔÓ˜ÚË Í‡Í Í‡ÏÂÌÌ˚È „ÓÎÂÏ) ----------------
