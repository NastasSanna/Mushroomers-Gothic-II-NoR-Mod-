...
func void B_CheerFight()
{
	...
	{
		zufall = Hlp_Random(3);
		if((C_NpcGetAttitude(self,victim) == ATT_FRIENDLY) && (C_NpcGetAttitude(self,other) != ATT_FRIENDLY))
		{
			if(zufall == 0)
			{
		...
		else
		{
			if(zufall == 0)
			{
				if (victim.aivar[AIV_Gender] == MALE)	{
					B_Say_Overlay(self,self,"$CHEERFRIEND01");
				}
				else	{
					B_Say_Overlay(self,self,"$CHEERFRIEND01_FV");
				};
			};
			if(zufall == 1)
			{
				if (victim.aivar[AIV_Gender] == MALE)	{
					...
					B_Say_Overlay(self,self,"$CHEERFRIEND02_FV");
				};
			};
			if(zufall == 2)
			{
				if (victim.aivar[AIV_Gender] == MALE)	{
					...
					B_Say_Overlay(self,self,"$CHEERFRIEND03_FV");
				};
			};
...
func void B_AssessDefeat()
{
	...
	{
		if(C_NpcGetAttitude(self,other) == ATT_FRIENDLY)
		{
			if (other.aivar[AIV_Gender] == MALE && victim.aivar[AIV_Gender] == MALE)	{
				B_Say(self,other,"$GOODVICTORY");
			}
			else if (victim.aivar[AIV_Gender] == FEMALE)	{
				B_Say(self,other,"$GOODVICTORY_FV");
			}
			else {
				B_Say(self,other,"$GOODVICTORY_F");
			};
		}
		else
		{
