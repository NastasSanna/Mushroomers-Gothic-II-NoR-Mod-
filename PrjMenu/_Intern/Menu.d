...
const int IT_EFFECTS_NEXT = 128;	//перелистывание связанного SLIDER
const int IT_ONLY_OUT_GAME = 256;	//только на стартовом экране
const int IT_ONLY_IN_GAME = 512;	//только в игре
const int IT_PERF_OPTION = 1 << 10;	//опции производительности ?
const int IT_MULTILINE = 1 << 11;	//многострочный текст
const int IT_NEEDS_APPLY = 1 << 12;	//требуется применить
const int IT_NEEDS_RESTART = 1 << 13;	//требуется перезгрузка
const int IT_EXTENDED_MENU = 1 << 14;	//только в расширенном меню
